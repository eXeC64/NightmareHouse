	...
	case HUDTYPE_CAROUSEL:
		{
			...
			else if ( m_WeaponBoxes.Count() < MAX_CAROUSEL_SLOTS )
			{
				...
				height = (m_WeaponBoxes.Count()-1) * (m_flLargeBoxWide+m_flBoxGap) + m_flLargeBoxTall;
				ypos  = (GetWide() - width)/2;
				for ( int i=0; i<m_WeaponBoxes.Count(); i++ )
				{
					...
					ypos += (m_flLargeBoxWide + m_flBoxGap);
				}
			}
			else
			{
				...
				ypos = GetWide()/2 + m_flHorizWeaponSelectOffsetPoint - largeBoxWide/2;
				...
				ypos = GetWide()/2 + m_flHorizWeaponSelectOffsetPoint - (3*largeBoxWide/2 + m_flBoxGap);
	...
	case HUDTYPE_BUCKETS:
		{
			...
			height = (MAX_WEAPON_SLOTS - 1) * (m_flSmallBoxSize + m_flBoxGap) + largeBoxTall;
			xpos  = 16;
			ypos  = (GetTall() - height)/2;
	...
	case HUDTYPE_BUCKETS:
		{
			...
			if ( pWeapon->GetSpriteActive() )
			{
				...
				else if (bSelected)
				{
					...
					surface()->DrawTexturedRect( xpos + m_flWepXPos , ypos + m_flWepYPos, xpos + m_flWepXPos + m_flWepXSize, ypos + m_flWepYPos + m_flWepYSize);
				}
			}
		}
		break;
